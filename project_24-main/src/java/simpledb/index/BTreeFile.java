...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid,
		...
		if (pid.pgcateg() == BTreePageId.LEAF) {
			// fetch page from buffer pool and return it, use wrapper function
			// BTreeFile.getPage()
			return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
		}
		// recurse, if internal page, get page with READ_ONLY permission
		else {
			BTreeInternalPage internalPage = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
			// get iterator from internal page
			Iterator<BTreeEntry> iter = internalPage.iterator();
			// init BTreeEntry
			BTreeEntry entry = null;
			// handle null f value, recurse to leftmost everytime
			if (f == null) {
			...
			while (iter.hasNext()) {
				entry = iter.next();
				// get entry key value and compare
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// Create a new sibling page and distribute half of the tuples from the current
		...
		int tupleTotal = page.getNumTuples();
		for (int i = 0; i < tupleTotal / 2; ++i) {
			if (!tupleIterator.hasNext()) {
				throw new DbException(null);
			}
			Tuple currentTuple = tupleIterator.next();
			page.deleteTuple(currentTuple);
			siblingPage.insertTuple(currentTuple);
		}

		// Update sibling pointers for the affected leaf pages
		if (page.getRightSiblingId() != null) {
			BTreePageId oldSiblingId = page.getRightSiblingId();
			BTreeLeafPage oldSiblingPage = (BTreeLeafPage) getPage(tid, dirtypages, oldSiblingId,
					Permissions.READ_WRITE);
			oldSiblingPage.setLeftSiblingId(siblingPage.getId());
		}

		siblingPage.setLeftSiblingId(page.getId());
		siblingPage.setRightSiblingId(page.getRightSiblingId());
		...
		// Create a new entry for the parent page with the middle key and divided leaf
		// page pointers
		Field middleKey = siblingPage.iterator().next().getField(keyField);
		BTreeEntry newEntry = new BTreeEntry(middleKey, page.getId(), siblingPage.getId());

		// Insert the new entry into the parent page and update parent pointers
		BTreeInternalPage parentNode = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleKey);
		parentNode.insertEntry(newEntry);
		updateParentPointers(tid, dirtypages, parentNode);

		// Return the appropriate leaf page for the provided key field
		return (field.compare(Op.GREATER_THAN_OR_EQ, middleKey) ? siblingPage : page);
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// field
		// should be inserted.
		// create a new empty internal page to copy over the tuples
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// some code goes here
		BTreeLeafPage rightNode;
		int numTupleToSteal = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		Tuple[] tupleToSteal = new Tuple[numTupleToSteal];
		int countdown = tupleToSteal.length - 1;
		...
		// If sibling is on the left
			...
			rightNode = page;
			// Use reverse iterator as you want to steal data from the end entries
			iterator = sibling.reverseIterator();
		}

		while (countdown >= 0 && iterator.hasNext()) {
			tupleToSteal[countdown--] = iterator.next();
		}

		// rotate keys through parent
		for (Tuple t : tupleToSteal) {
			sibling.deleteTuple(t);
			page.insertTuple(t);
		}

		if (rightNode.getNumTuples() > 0) {
			entry.setKey(rightNode.iterator().next().getField(keyField));
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry[] entriesToMoveArray = new BTreeEntry[entriesToMove];

		// Use the reverse iterator to steal the last entries
		Iterator<BTreeEntry> iterator = leftSibling.reverseIterator();
		int countdown = entriesToMoveArray.length - 1;

		while (countdown >= 0 && iterator.hasNext()) {
			entriesToMoveArray[countdown--] = iterator.next();
		}

		for (int i = entriesToMoveArray.length - 1; i >= 0; --i) {
			BTreeEntry entry = entriesToMoveArray[i];
			...
			// The entry from the left sibling must be inserted into the first position in
			// the right page
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// make a copy to avoid copying while modifying
		while (counter < entriesToMoveArray.length && iterator.hasNext()) {
			entriesToMoveArray[counter++] = iterator.next();
		}

		for (int i = 0; i < entriesToMoveArray.length; ++i) {
			BTreeEntry entry = entriesToMoveArray[i];
			// rotate through the parent
			...
			// entry is shifted up to be the root; update its children along with the record
			// ID
			entry.setLeftChild(parentEntry.getLeftChild());
			entry.setRightChild(parentEntry.getRightChild());
			entry.setRecordId(parentEntry.getRecordId());
			...
			parentEntry.setRightChild(pageId);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// some code goes here
		Tuple[] tuplesToDelete = new Tuple[rightPage.getNumTuples()];
		int deletionCount = tuplesToDelete.length - 1;
		Iterator<Tuple> iterator = rightPage.iterator();
		while (deletionCount >= 0 && iterator.hasNext()) {
			tuplesToDelete[deletionCount--] = iterator.next();
		}

		// Remove tuples from the right page and insert them into the left page
		for (Tuple tuple : tuplesToDelete) {
			rightPage.deleteTuple(tuple);
			leftPage.insertTuple(tuple);
		}
		// Update sibling pointers by setting the left page's right sibling to the right
		// page's right sibling
		...
		// Add the left page to the modified pages for flushing
		dirtypages.put(leftPage.getId(), leftPage);

		// Mark the right page as empty and available for reuse
		setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber());

		// Delete the parent entry pointing to the old pages since they have been merged
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Update the parent entry to point to the new children after deleting and
		// pulling it down
		...
		// For each entry in the right page, delete it and update its children's parent
		// to the left page
		// since the right page is being merged with the left page.
		...
		// we mark the left page as dirty so we can flush to disk
		dirtypages.put(leftPage.getId(), leftPage);
		setEmptyPage(tid, dirtypages, rightPage.getId().getPageNumber());
